...
- **选片工作流** — 扫描文件夹内 JPG，预览全分辨率照片，`Space` 选中照片，`A` 标记保留、`D` 标记删除，一键批量移动保留的 JPG（连带 RAW 文件）到目标文件夹，同时删除标记为删除的文件
...
1. **选片** — 进入选片标签页，`Space` 选中照片，`A` 标记保留、`D` 标记删除，`Ctrl+M` 批量移动
